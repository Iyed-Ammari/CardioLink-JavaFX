...
public class PatientPanierController implements Initializable {
    ...
    private void initialiserVue() {
        ...
        if (cartScrollPane != null) {
            cartScrollPane.setFocusTraversable(false);
        }
    }

    private void chargerPanier() {
        try {
            panierCourant = commandeService.getOrCreatePanier(
    ...
    private void afficherLignes(List<LigneCommande> lignes) {
        ...
        for (LigneCommande l : lignes) {
            cartContainer.getChildren().add(creerCarteLigne(l));
        }
    ...
    private VBox creerCarteLigne(LigneCommande ligne) {
        ...
        String nomTxt = ligne.getProduit() != null && ligne.getProduit().getNom() != null
    ...
    private void mettreAJourResume() {
        int nb = panierCourant != null && panierCourant.getLignes() != null
                ? panierCourant.getLignes().size()
                : 0;

        BigDecimal total = panierCourant != null && panierCourant.getMontantTotal() != null
                ? panierCourant.getMontantTotal()
                : BigDecimal.ZERO;
    ...
    private void handleValiderCommande() {
        ...
        try {
            commandeService.validerCommande(panierCourant);
    ...
    @FXML
    private void handlePayerCommande() {
        try {
            if (panierCourant == null) {
                showError("Panier introuvable.");
                return;
            ...
            if (panierCourant.getStatut() == Commande.Statut.PANIER) {
                commandeService.validerCommande(panierCourant);
                panierCourant = commandeService.getById(panierCourant.getId());
            }

            if (panierCourant.getStatut() != Commande.Statut.EN_ATTENTE_PAIEMENT) {
                showError("La commande doit être en attente de paiement.");
                return;
            }
            ...
            paiementService.payerCommandeAvecStripe(
                    panierCourant,
                    () -> {
                        showInfo("✅ Paiement Stripe confirmé.");
                        goToCommandes();
                    },
                    this::showInfo,
                    this::showError
    ...
    private void goToCommandes() {
        try {
            NavigationUtil.navigate((Stage) cartContainer.getScene().getWindow(), "/fxml/patient/commande-list-patient.fxml");
        } catch (Exception e) {
            showError("Navigation impossible.");
        }
    }
